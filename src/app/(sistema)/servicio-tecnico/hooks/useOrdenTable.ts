...
import { OrdenServicio } from "@/app/(sistema)/servicio-tecnico/types/orden/Orden";
 ...
 */
const filterFn = (data: OrdenServicio[], values: Record<string, string>) => {
  return data.filter((o) => {
    // Buscador de texto
    const matchSearch =
      !values.search ||
      o.orden_servicio.toLowerCase().includes(values.search.toLowerCase()) ||
      o.servicio_tecnico.toLowerCase().includes(values.search.toLowerCase()) ||
      o.cliente.toLowerCase().includes(values.search.toLowerCase());

    // Rango de Fechas (Desde - Hasta)
    let matchFecha = true;
    if (values.fechaDesde || values.fechaHasta) {
      const dateC = parse(o.fecha_registrada, DATE_FORMAT_DATA, new Date());
...
/**
 * HOOK PERSONALIZADO: useOrdenTable
 ...
 */
export function useOrdenTable(data: OrdenServicio[]) {
  return useTableData({
    data,
    filterFn,
    initialFilters: {
