...
import { MotivoList } from "./MotivoList";

interface MotivoModalProps {
  manager: MotivoManager;
}

export function MotivoModal({ manager }: MotivoModalProps) {
  return (
    <Dialog open={manager.isOpen} onOpenChange={manager.handleOpenChange}>
      <DialogContent
        showCloseButton={false}
        className="flex max-h-[92vh] !w-[calc(100vw-2rem)] !max-w-[calc(100vw-2rem)] flex-col gap-0 overflow-hidden rounded-[3px] bg-white p-0 font-sans sm:!max-w-[850px]"
      >
        {/* Header */}
        <DialogHeader className="flex-row items-center justify-between border-b border-gray-200 px-5 py-3.5">
          ...
          <DialogDescription className="sr-only">
            Administra los motivos de compra, venta o traslado.
          </DialogDescription>
          <DialogClose asChild>
            <button
              type="button"
        ...
        <div className="flex flex-col gap-5 overflow-y-auto p-5 sm:p-6">
          <MotivoForm
            key={manager.editingMotivo?.id ?? "new"}
            manager={manager}
          />

          <div className="border-t border-gray-200" />

          <MotivoList manager={manager} />
        </div>
      </DialogContent>
    </Dialog>
  );
