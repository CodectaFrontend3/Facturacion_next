// _components/documentos/form/ArticuloSelectorModal.tsx
"use client"
...
interface ArticuloSelectorModalProps {
  isOpen: boolean
  onClose: () => void
  onAdd: (articuloId: string, cantidad: number) => void
...
/**
 * ArticuloRow: fila memoizada para evitar re-renders innecesarios de toda la tabla.
 */
    ...
    >
      ...
      <td className="py-2.5 px-2 text-center">
        <input
          ...
          onChange={(e) => onCantidadChange(String(articulo.id), Math.max(1, Number(e.target.value)))}
        />
      </td>
...
export function ArticuloSelectorModal({ isOpen, onClose, onAdd, articulosMaster }: ArticuloSelectorModalProps) {
  ...
  const filtered = useMemo(() => {
    return articulosMaster.filter(
      (a) =>
        a.nombre.toLowerCase().includes(search.toLowerCase()) ||
        a.codigo.toLowerCase().includes(search.toLowerCase())
    )
  }, [search, articulosMaster])
  ...
  const handleCantidadChange = useCallback((id: string, val: number) => {
            ...
            <tbody>
              {pageItems.map((a) => (
                <ArticuloRow
                  key={a.id}
                  articulo={a}
                  cantidad={cantidades[String(a.id)] || 1}
