...
import { NotaCreditoRow } from "../page";

export default function DetalleNotaCreditoPage() {
  const { id } = useParams();
  const router = useRouter();
  const [nota, setNota] = useState<NotaCreditoRow | null>(null);

  useEffect(() => {
    const guardadas: NotaCreditoRow[] = JSON.parse(localStorage.getItem('notas_credito_guardadas') || '[]');
      ...
      <div className="w-full bg-white border border-gray-200 shadow-sm relative overflow-hidden">
        ...
        <div className="px-8 py-6 flex items-center justify-between border-b border-gray-100 relative">
          ...
          <div className="absolute left-0 w-full text-center z-0 pointer-events-none">
            <h1 className="text-[22px] font-light text-[#ed5565] tracking-wider">NOTA DE CRÉDITO ELECTRÓNICA</h1>
          </div>
        </div>
        ...
        <div className="p-8">
          <div className="grid grid-cols-2 gap-4 mb-8">
            ...
            <div className="border border-gray-200 p-4 rounded-sm text-[12px] leading-relaxed bg-red-50/30">
              <p><span className="font-bold text-[#ed5565]">Documento que Modifica:</span> <span className="font-bold">{nota.docModifica}</span></p>
            ...
            <table className="w-full text-left text-[11px]">
              ...
              <tbody>
                <tr className="border-b border-gray-100 text-gray-600">
                  ...
                  <td className="py-3 px-2">NC-ITEM-001</td>
                  <td className="py-3 px-2">ANULACIÓN / AJUSTE DE OPERACIÓN POR {nota.docModifica}</td>
          ...
          <div className="flex justify-between items-start mb-8">
            <div className="w-1/2">
              ...
              <p className="text-[11px] text-gray-500 mt-2">Documento emitido según normativa SUNAT de comprobantes de pago electrónicos.</p>
            </div>
            ...
            <div className="w-80 border border-gray-200 p-4 rounded-sm text-[12px]">
              ...
              <div className="flex justify-between"><span className="text-[#ed5565] font-bold">Importe Total Ajuste:</span><span className="font-bold text-[#ed5565]">S/ {importeNumerico.toFixed(2)}</span></div>
            </div>
          </div>
        </div>
      </div>
