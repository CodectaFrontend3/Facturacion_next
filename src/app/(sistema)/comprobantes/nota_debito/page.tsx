...
const columns: ColumnDef<NotaDebitoRow>[] = [
  ...
  { 
    ...
    cell: ({ row }) => (
      <div className="flex items-center gap-2 whitespace-nowrap">
        <span className="font-semibold text-[#f8ac59]">{row.original.nro}</span>
  ...
  { 
    ...
    cell: ({ row }) => (
      <div className="flex justify-center">
        <Link href={`/comprobantes/nota_debito/${row.original.id}`}>
  ...
  { 
    ...
    cell: () => (
      <div className="flex justify-center items-center gap-1.5">
        ...
        <div className="bg-[#f8ac59] text-white w-6 h-6 rounded-full flex items-center justify-center shadow-sm"><i className="bi bi-file-earmark-plus text-[11px]"></i></div>
      </div>
    )
  },
  { 
...
export default function NotaDebitoPage() {
  const [data, setData] = useState<NotaDebitoRow[]>([]);
