export function montoSinSimbolo(value?: string) {
  return (value || "0.00").replace(/S\/|\$/g, "").replace(/,/g, "").trim()
}

export function monedaDesdeImporte(value?: string) {
  return value?.trim().startsWith("$") ? "Dólares" : "Soles"
}

export function simboloDesdeMoneda(moneda: string) {
  return moneda.toLowerCase() === "soles" ? "S/" : "$"
}

function unidades(u: number) {
  switch (u) {
    case 1: return "un"
    case 2: return "dos"
    case 3: return "tres"
...
export function numeroALetras(numStr?: string, moneda: string = "Dólares"): string {
  ...
  const monName = moneda.toLowerCase() === "soles" ? "Soles" : "Dólares"
