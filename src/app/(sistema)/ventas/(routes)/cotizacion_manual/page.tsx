// (routes)/cotizacion_manual/page.tsx
"use client"
...
import { getCotizacionManualColumns } from "../../_config/columns/cotizacionManual.columns"
...
export default function CotizacionManualPage() {
  ...
  const manualesFiltradas = useVentasList(cotizacionesManuales, activeFilters)
  ...
  const totalAmount = useMemo(
    () => format.moneda(manualesFiltradas.reduce((acc, r) => acc + (r.total ?? 0), 0)),
    [manualesFiltradas]
  )
  ...
  const columns = getCotizacionManualColumns({
    ...
    onView: (doc) => router.push(`${basePath}/cotizacion_manual/${doc.id}`),
  })
  ...
  return (
    <>
      <VentasListLayout
        activeTab="cotizacionesManuales"
        summaryCards={summaryCards}
        tableColumns={columns}
        tableData={manualesFiltradas}
        tabCounts={tabCounts}
        totalAmount={totalAmount}
        onAddClick={() => router.push(`${basePath}/cotizacion_manual/crear`)}
        filterBar={
          <FilterBar
            filters={filters}
