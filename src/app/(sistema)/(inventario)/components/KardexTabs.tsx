...
function KardexTabs({ children, actions }: Props) {
  ...
  // Filtramos para obtener únicamente la pestaña que haga match con la URL actual
  const activeTab = allTabs.find((tab) => pathname === tab.href);

  return (
    <div className="w-full min-h-screen bg-[#f8fafc] p-4 lg:p-6">
      {/* Cabecera: El Tab Activo Solitario + Acciones */}
      <div className="flex items-center justify-between">
        <nav className="flex items-end space-x-1">
          {activeTab ? (
            ...
            /* Fallback visual por si entras a una subruta no mapeada directamente */
        ...
        {/* Botones de acción dinámicos a la derecha */}
