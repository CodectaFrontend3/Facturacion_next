...
// Función de filtrado
const filterFn = (data: KardexSalidaRow[], values: Record<string, string>) => {
  return data.filter((c) => {
    ...
    if (values.fechaDesde || values.fechaHasta) {
      const dateC = parse(c.fecha_salida, DATE_FORMAT_DATA, new Date());
...
export function useSalidaTable(data: KardexSalidaRow[]) {
