...
const iconMap: Record<string, string> = {
  ...
  Users:
    "http://jypsac.dyndns.org:190/facturacion_20522045773/public/archivos/imagenes/layout/auxiliar.svg",
};

const isUrl = (str: string) =>
  /^(https?:\/\/|\/)/.test(str) || str.endsWith(".svg");

const mainItems = sidebarData as sidebarItem[];

// Lógica mejorada: Evaluamos si pertenece al mismo módulo padre
function isActiveRoute(
  ...
  // Si estamos evaluando el menú principal (Padre), verificamos si la URL actual
  // y el link del menú comparten el mismo directorio base (ej. "/comprobantes")
  if (isParent) {
    const pathModule = pathname.split("/")[1];
    const hrefModule = href.split("/")[1];
    if (pathModule && hrefModule && pathModule === hrefModule) {
      return true;
    }
  }

  // Para los submenús, debe ser coincidencia exacta o ser una ruta hija directa (ej. /crear)
  return pathname === href || pathname.startsWith(href + "/");
}

function getActiveMenuLabel(pathname: string, items: sidebarItem[]) {
  const activeItem = items.find((item) => {
    if (item.path) return isActiveRoute(pathname, item.path, true); // Evaluamos como padre
    if (item.subPath)
      return item.subPath.some((sub) => {
        if (sub.path) return isActiveRoute(pathname, sub.path, true);
        if (sub.kPath)
          return sub.kPath.some((k) => isActiveRoute(pathname, k.path, true));
        return false;
      });
    return false;
  });
...
export default function AppSidebar() {
  ...
  const activeMenuLabel = useMemo(
    () => getActiveMenuLabel(pathname, mainItems),
    [pathname],
  );
  const resolvedOpenMenu =
    openMenuState && openMenuState.pathname === pathname
      ? openMenuState.label
      : activeMenuLabel;

  const menuList = useMemo(() => {
    return mainItems.map((item) => {
      // Evaluamos la activación del elemento principal (isParent = true)
      const active = item.path
        ? isActiveRoute(pathname, item.path, true)
        : item.subPath?.some((sub) => {
            if (sub.path) return isActiveRoute(pathname, sub.path, true);
            if (sub.kPath)
              return sub.kPath.some((k) =>
                isActiveRoute(pathname, k.path, true),
              );
            return false;
          });
      ...
      const subItems = item.subPath?.map((subItem) => {
        // Evaluamos la activación del submenú (isParent = false)
        const subActive = subItem.path
          ? isActiveRoute(pathname, subItem.path, false)
          : subItem.kPath?.some((k) => isActiveRoute(pathname, k.path, false));

        const kItems = subItem.kPath?.map((kItem) => {
          const kActive = isActiveRoute(pathname, kItem.path, false);
          return {
            ...kItem,
            active: kActive,
          };
                      ...
                      <CollapsibleTrigger asChild>
                        ...
                        >
                          {isUrl(iconClass) ? (
                            ...
                            <i
                              className={`${iconClass} flex! items-center! justify-center! shrink-0 transition-all duration-500 ease-in-out w-4 h-4.25 text-[17px]! group-data-[collapsible=icon]:w-4.5 group-data-[collapsible=icon]:h-5.25 group-data-[collapsible=icon]:text-[21px]!`}
                            />
                          )}
                      ...
                      <CollapsibleContent className="overflow-hidden data-[state=closed]:animate-[collapsible-up_0.2s_ease-out] data-[state=open]:animate-[collapsible-down_0.2s_ease-out]">
                        <SidebarMenuSub className="m-0 p-0 gap-0 border-none space-y-0 flex-col translate-x-0 mx-0 px-0">
                          {item.subPath.map((subItem) => {
                            return subItem.kPath ? (
                              <Collapsible
                                key={subItem.label}
                                className="group/subcollapsible transition-all duration-200"
                              >
                                <SidebarMenuSubItem>
                                  <CollapsibleTrigger asChild>
                                    <SidebarMenuSubButton
        ...
        <div className="flex w-59 items-center gap-3 px-2 py-2 text-left text-white transition-all duration-500 ease-in-out group-data-[collapsible=icon]:opacity-0 group-data-[collapsible=icon]:invisible">
          ...
          >
            <DropdownMenuTrigger asChild>
              <button
                type="button"
                className="rounded-md p-1.5 transition-colors hover:bg-white/10 focus:outline-none cursor-pointer"
              >
            ...
            >
              <DropdownMenuItem
                asChild
                className="text-[12px] font-bold px-3 py-1.5 transition-all duration-200 hover:pl-4 focus:pl-4 hover:bg-gray-100 focus:bg-gray-100 cursor-pointer text-black"
              >
